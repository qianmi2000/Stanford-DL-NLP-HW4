...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, self.hidden_size,dropout = self.dropout_rate,bidirectional=True)   #bi-directional LSTM as encoder
        ...
        self.c_projection = nn.Linear(self.hidden_size*2, self.hidden_size,bias = False) #同h_projection; 将encoder两个hidden拼接转化为decoder的初始hidden (2h,1)->(h,1)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        #init_decoder_cell = selfpack_padded_sequence.c_projection(torch.cat((last_cell[0],last_cell[1]),dim = 1)) #拼接后转化
